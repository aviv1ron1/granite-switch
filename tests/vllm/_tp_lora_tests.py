...
def _make_layer(
    ...
    with (
        ...
        patch("granite_switch.vllm.core.lora.get_tensor_model_parallel_rank", return_value=tp_rank),
    ):
...
class TestPackedSlicing:
    ...
    def test_packed_b_both_ranks_reconstruct(self):
        ...
        for i, full_out in enumerate(slice_sizes):
            w = torch.randn(NUM_ADAPTERS, 1, full_out, RANK)
            r0 = _make_layer(
                True, False, tp_size=2, tp_rank=0, num_slices=3, output_slices=slice_sizes
            ).slice_lora_b_weight(w, slice_idx=i)
            r1 = _make_layer(
                True, False, tp_size=2, tp_rank=1, num_slices=3, output_slices=slice_sizes
            ).slice_lora_b_weight(w, slice_idx=i)
            assert torch.equal(
                torch.cat([r0, r1], dim=-2), w
