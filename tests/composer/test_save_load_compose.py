...
def phase1(tmp_path_factory):
    ...
    built_tokenizer_ids = {
        tok: tokenizer.convert_tokens_to_ids(tok) for tok in tokenizer.all_special_tokens
    }
    ...
    built_golden_encoded = [tokenizer.encode(s, add_special_tokens=False) for s in golden_strings]

    rng = random.Random(SEED)
    sample_size = min(100, original_vocab_size)
    sample_ids = rng.sample(range(original_vocab_size), sample_size)
    built_base_vocab_sample = {tid: tokenizer.convert_ids_to_tokens(tid) for tid in sample_ids}
...
def phase2(tmp_path_factory):
    ...
    tok1_control_ids = {
        tok: tokenizer_1.convert_tokens_to_ids(tok) for tok in tokenizer_1.all_special_tokens
    }

    adapter_name_for_golden = loaded_1.config.adapter_names[0]
    golden_strings = _make_golden_set(adapter_name_for_golden)
    tok1_golden_encoded = [tokenizer_1.encode(s, add_special_tokens=False) for s in golden_strings]
    ...
    tok2_control_ids = {
        tok: tokenizer_2.convert_tokens_to_ids(tok) for tok in tokenizer_2.all_special_tokens
    }

    tok2_golden_encoded = [tokenizer_2.encode(s, add_special_tokens=False) for s in golden_strings]
...
class TestPhase1_BuildSaveLoad:
    ...
    def test_pipeline_metadata_files_exist(self, phase1):
        ...
        assert not (
            save_dir / "README.md"
        ).exists(), "Upstream README.md should not be copied into the composed output"
    ...
    def test_tokenizer_control_token_ids(self, phase1):
        ...
        for i, name in enumerate(adapter_names):
            ...
            assert (
                loaded_ids[act_tok] == act_expected
    ...
    def test_tokenizer_golden_set(self, phase1):
        ...
        mismatches = []
        for s, b, lo in zip(strings, built, loaded, strict=False):
            if b != lo:
                mismatches.append((s, b, lo))

        if mismatches:
            for s, b, lo in mismatches:
    ...
    def test_tokenizer_atomic_control_tokens(self, phase1):
        ...
        for name in adapter_names:
            ...
            if not is_atomic:
                ...
                non_atomic.append(f"{control_token} → {len(encoded)} tokens: {sub_tokens}")
...
class TestPhase2_DoubleSerialization:
    ...
    def test_tokenizer_vocab_size(self, phase2):
        ...
        assert len_2 == len_1, (
            f"Vocab size differs: loaded_1={len_1}, loaded_2={len_2} " f"(diff={len_2 - len_1})"
        )
    ...
    def test_tokenizer_golden_set(self, phase2):
        ...
        mismatches = []
        for s, a, b in zip(strings, enc_1, enc_2, strict=False):
            if a != b:
