...
def discover_adapters_from_yaml(manifest_path: str) -> list[tuple[str, str, str, str | None]]:
...
def _report_module_contributions(
    ...
    for g in sorted(arch.groups, key=lambda g: g.name):
        contributors = [name for name, info in adapter_modules.items() if g.name in info["groups"]]
        if contributors:
            print(
...
def list_available_adapters(
    ...
    return [
        {"name": name, "technologies": sorted(techs)} for name, techs in sorted(by_name.items())
    ]
...
def _list_repo_adapter_names(repo_id: str) -> list[str]:
    ...
    return [
        item.path for item in tree if isinstance(item, RepoFolder) and not item.path.startswith("_")
    ]
...
def _resolve_technology(
    ...
    try:
        ...
        technologies = {
            item.path.split("/")[-1]
            for item in subtree
            if isinstance(item, RepoFolder) and item.path.split("/")[-1] in ("alora", "lora")
        }
...
def _build_allow_patterns(
    ...
    if include_adapters:
        adapter_names = [
            name for name in adapter_names if any(fnmatch(name, pat) for pat in include_adapters)
        ]
...
def resolve_repo_path(
    ...
    if "/" in path_or_repo and not local.exists():
        ...
        allow_patterns = None
        if target_model_name or include_adapters or exclude_adapters or technology_filter:
            try:
                ...
                print(
                    f"  WARNING: Failed to build download filters ({e}), " f"downloading full repo"
                )
                allow_patterns = None
