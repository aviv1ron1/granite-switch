...
def _format_adapters_section(
    ...
    for entry, rank, alpha, tgt, source in zip(
        adapters, ranks, alphas, targets, sources, strict=False
    ):
...
def _format_composition_details_section(
    ...
    if compose_settings:
        visible_settings = {
            k: v for k, v in compose_settings.items() if v is not None and v != "" and v != []
        }
    ...
    if base_param_count is not None and composed_param_count is not None and base_param_count > 0:
...
def write_build_doc(
    ...
    adapter_sources = [_short_source(t[3] if len(t) > 3 else None) for t in all_discovered]
    ...
    compose_settings = {
        ...
        "adapter_substitute_token_ids": getattr(model.config, "adapter_substitute_token_ids", None),
    ...
    if getattr(args, "built_in_adapters", None):
        built_in_alpha = float(args.lora_alpha if args.lora_alpha is not None else args.lora_rank)
