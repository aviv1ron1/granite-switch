...
"""Export a Granite Switch model to a single ONNX decode graph + a CLI.

Produces a self-contained model directory runnable by ``onnxruntime`` /
``onnxruntime-web``:

    <out>/decode.onnx       input_ids [1,N] + past_* -> logits, present_*
    <out>/gs_onnx.json      metadata the JS runtime needs (layers, kv heads, hd, etc.)

ONE graph serves both passes: the batched first pass over the whole prompt
(``input_ids [1,N]`` with an empty past) and incremental single-token decode
(``input_ids [1,1]`` with a non-empty past). ``input_ids`` seq-length is a dynamic
axis and :func:`granite_switch.onnx.decode._forward_core` is generalized over it,
so there is no separate prefill graph (which would duplicate the same weights).
...
from .wrapper import reskin_for_export
...
# ``decode.onnx`` embeds when the whole thing fits comfortably under the cap
# (one file, convenient for onnxruntime / onnxruntime-node, which resolve a
# sidecar automatically anyway).
#
...
def _embed_external_data(onnx_path):
    ...
    file. For the top-level ``decode.onnx`` of small models we fold the weights
    back into one ``.onnx`` file for a convenient single-file artifact
    (``onnxruntime`` / ``onnxruntime-node`` load it directly).
...
def _keep_external_data(onnx_path):
    ...
    exporter's sidecar name varies (``decode.onnx.data`` vs ``decode.data``),
...
def export_decode(model, output_path, *, n_layers, kv_heads, head_dim, opset=OPSET, embed=True):
    """Export the single KV-cached graph that serves BOTH prefill and decode.

    ``input_ids`` seq-length is a dynamic axis, so this one graph runs:

    * the batched FIRST pass over the whole prompt — ``input_ids [1, N]`` with an
      EMPTY past (``past_len=0``); the browser runtime feeds zero-length ``past_*``
      tensors (see ``web/src/granite-switch-register.js``'s ``emptyCacheFeeds``), and
    * incremental decode — ``input_ids [1, 1]`` with a non-empty past.

    :func:`_forward_core` is generalized over ``q_len``, so a single exported graph
    covers both. This replaces the former separate ``prefill.onnx`` (which shipped a
    duplicate copy of the same weights).
    ...
    bsz = 1
    # Example mixes a >1 query length with a >0 past so the export traces the
    # general (q_len>=1, past_len>=0) case both the first pass and decode hit.
    ex = [
    ...
    dynamic_shapes = ({1: seq_len}, {1: past_len}, {1: past_len}, past_kv_shapes)
...
def export_model_dir(model, out_dir, *, opset=OPSET, embed=None):
    """Export the single decode graph and metadata into ``out_dir``.

    ``embed`` controls whether the TOP-LEVEL ``decode.onnx`` folds weights inline
    (one ``.onnx`` file) or keeps an external ``.onnx.data`` sidecar. ``None``
    (default) auto-selects: embed when the model fits under the ~2 GiB protobuf
    cap, otherwise keep the sidecar (required for large models like the 4 B
    ...
    # ONE graph for both the batched first pass and incremental decode
    # (export_decode reskins the model in place and makes input_ids dynamic).
    export_decode(
    ...
    #
    # The tfjs graph is ALWAYS external (graph + .onnx.data sidecar), regardless
    ...
    # loads this single graph for both the batched first pass and decode.
    ...
    # Browser-load hints: which tfjs artifact the JS runtime should fetch, and its
    # external-data sidecar name (the basename baked into the graph's `location`,
    # which the `externalData` session option must match).
...
def main():
    ...
    print(f"Exported decode graph → {out}")
